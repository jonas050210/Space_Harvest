...
| Rendering | Ursina scene: inverted starfield-sprite skybox, layered sun corona billboards, unlit textured planets with billboard name tags, double ring on Aurelia, the whole asteroid belt baked into a single mesh (one draw call), per-class ship silhouettes (needle scout, barge hauler, drummed refinery) with tints and textured engine flares, fading trails, smoothed chase camera, click-to-target picking, pulsing target reticle, quality presets. |
| Game shell | Keyboard-navigable title menu (new game / continue / load / settings / how-to-play / quit), pause menu with quit-to-title, persistent settings (quality preset, audio, camera glide) in a dedicated savegame slot, 3-page how-to-play, toast stack, launch-window GO moment (blinking banner + chime), NEXT WINDOWS board counting down every route. |
...
| Comet Vigil | A long-period eccentric comet (e=0.8) as a campaign-only body: rare expensive windows, primordial ice + platinum veins, anti-sunward tail with inverse-square brightness, own windows-board entry. |
| Rare ores | Thorite (deep belt, derelict hull, comet slag) and aurellium (comet-only jackpot) trade at tight absorption; campaign rarity maps register at mining import so fingerprints are correct from boot. |
| Refinery stations | Player-built smelters (E): a run's payload is smelted at docking (components from iron+silver, electronics from gold) and drone-loaded ore keeps smelting during the wait; refined stock sells far above raw ore. |
| Science unlocks | Research points commission four one-shot technologies (parts discount, softer crew fatigue, faster depot ISRU, quicker smelting) applied to the sim as generic multipliers; purchases and multipliers persist with the campaign; `[L]` in play. |
| Firsts milestones | 19 one-shot KSP-style achievements (first captures per body, comet encounter, stations, drones, full-hold returns, tonnage, treasury, rare-ore shipments) paying credits + research; latched once-only, saved with the campaign. |
| Parts market & upgrades | Drop Tanks / Deep Drill / Crew Quarters / Depot Drone Bay / aurellium-gated Navigation Suite (ops-layer planning refund) at seasonally swinging, count-escalating Earth prices; T/Y/U refit the least-upgraded docked ship, P installs drone bays that honestly mine waiting ships' holds full from the shared depletion ledgers. |
